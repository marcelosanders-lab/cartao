...
Caching:
  ...
  Cache persists until explicitly cleared or source PDF changes.
    ...
    # --- Guarda contra PDF digitalizado (adicionado; ver scripts/scan_guard.py) ---
    # Upstream grava um .md vazio em silêncio quando o PDF não tem camada de
    # texto. Para uso probatório isso é inaceitável: o modelo conclui a partir
    # do nada. Aqui a ausência de texto vira OCR ou aviso explícito.
    import scan_guard

    scan_banner = ""
    if scan_guard.is_scanned_pdf(args.input, total_pages):
        # PDF digitalizado. Duas possibilidades a partir daqui:
        #  (a) o pymupdf4llm ja aplicou OCR por conta propria (faz isso em
        #      silencio quando o Tesseract esta instalado) -> so avisar;
        #  (b) nao ha texto nenhum -> tentar OCR aqui, ou avisar em bloco.
        available, reason = scan_guard.ocr_available()
        if not scan_guard.looks_scanned(result or "", total_pages):
            scan_banner = scan_guard.BANNER_OCR_USED
        elif available:
            print(
                "[pdf2md] Sem camada de texto; aplicando OCR...",
                file=sys.stderr,
            )
            ocr_config = ExtractionConfig(
                pdf_path=args.input, docling=args.docling, ocr=True
            )
            ocr_valid, ocr_key = cache_mgr.is_valid(ocr_config)
            ocr_cached = cache_mgr.load(ocr_key) if ocr_valid else None
            if ocr_cached:
                result = ocr_cached.markdown
                scan_banner = scan_guard.BANNER_OCR_USED
            else:
                try:
                    ocr_text = scan_guard.ocr_pdf(args.input, show_progress=show_progress)
                except Exception as e:
                    print(f"[pdf2md] OCR falhou: {e}", file=sys.stderr)
                    ocr_text = ""
                if scan_guard.looks_scanned(ocr_text, total_pages):
                    print(
                        "[pdf2md] OCR nao produziu texto utilizavel.",
                        file=sys.stderr,
                    )
                    scan_banner = scan_guard.BANNER_NO_OCR
                else:
                    result = ocr_text
                    scan_banner = scan_guard.BANNER_OCR_USED
                    cache_mgr.save(
                        ocr_key or cache_mgr.get_key(ocr_config),
                        ExtractionResult(
                            markdown=result, image_dir=None, total_pages=total_pages
                        ),
                        ocr_config,
                    )
        else:
            print(
                f"[pdf2md] AVISO: PDF sem camada de texto e OCR indisponivel ({reason}).",
                file=sys.stderr,
            )
            scan_banner = scan_guard.BANNER_NO_OCR
    ...
    if scan_banner is scan_guard.BANNER_NO_OCR:
        sys.exit(scan_guard.SCAN_EXIT_CODE)
